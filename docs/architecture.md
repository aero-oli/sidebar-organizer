...
One backend watcher observes the shared file and profile directory, including manual YAML edits. WebSocket subscriptions notify shared editors and every user whose effective profile changed. Browsers do not poll the filesystem. Personal collapsed-group state is debounced to a dedicated preferences endpoint and follows the Home Assistant user across devices.
